...
The Nova to Titan code swap and the self-sufficient seed load both touch shared `data_utils.py` copies and the seed data itself. Confirm before implementing: should the swap regenerate the provided `financial_data_load/seed-data/chunks.jsonl` embeddings with Titan (they were produced with Nova), or is the intent to keep the stored vectors and only change the query-time embedder to match? These must use the same model, or vector search returns garbage.
