...
### Lab 5: Agent Memory Notebooks (Verified end-to-end 2026-07-11)
...
- **Status:** Done and verified end-to-end. Both notebooks (`01_short_term_memory.ipynb`, `02_long_term_memory.ipynb`), `lib/memory_utils.py`, `lib/data_utils.py`, and `lib/__init__.py` are authored; `README.md`, `lab5.adoc`, and `lab5-instructions.adoc` are updated; the reasoning-traces "Going Further" callout is on the site page; nav lists Lab 5. Both notebooks now pass under `setup/run_notebooks.py --labs 5` against live Aura + Bedrock.
- **Runner registration:** Lab 5 is wired into `setup/run_notebooks.py`. `"5"` is in `_KNOWN_LABS`, both notebooks are in the `NOTEBOOKS` registry, and `neo4j-agent-memory[bedrock]==0.5.0` is in the PEP 723 header. `setup/README.md` scope now reads "Labs 3, 4, 5, and 6, plus the appendix."
- **Bug found and fixed (embedder config):** The first live run crashed in `build_memory_settings()`. `MemorySettings` validates its `embedding` argument against the runtime-checkable `EmbeddingProvider` protocol, which requires both `embed` and `embed_one`. `BedrockEmbedder` from `neo4j_agent_memory.embeddings` implements `embed` / `embed_batch` / `dimensions` but not `embed_one`, so it is rejected with `TypeError: MemorySettings.embedding must be EmbeddingConfig, a provider string, or an EmbeddingProvider instance; got BedrockEmbedder.` Fixed in `lib/memory_utils.py` by configuring the embedder as `EmbeddingConfig(provider=EmbeddingProvider.BEDROCK, model="amazon.titan-embed-text-v2:0", dimensions=1024, aws_region=REGION)`. `EmbeddingConfig.aws_region` keeps the region sourced from `CONFIG.txt`'s `REGION` key, so the earlier reason for avoiding the `bedrock/<model>` provider string still holds. `dimensions=1024` is required: the default is 1536, which would build a memory vector index that Titan V2's 1024-dim vectors cannot populate.
- **Deprecation switched:** The two inspect cells used `memory.graph.execute_read(...)`, which works in 0.5.0 but is deprecated and scheduled for removal in v0.6.0. Both now call `memory.query.cypher(...)`, the supported read-only path.
- **Open item (recall demo returns empty):** In `02_long_term_memory.ipynb` the recall cell runs clean but prints nothing. `search_entities("technology company", limit=5)` and `long_term.get_context("What do we know about Apple?")` both return empty. `search_entities` defaults to `threshold=0.7`, and at recall time the only stored entity is "Apple", whose embedding similarity to "technology company" lands below 0.7. The cell passes the runner because it never raises, but the notebook's headline "knowledge comes back in a fresh session" moment shows no output. Candidate fixes: lower the threshold, use query text closer to the stored entity, or reorder so `adopt_existing_graph` populates the entity set before the recall. Decision pending discussion.
...
- **Bolt path for Aura:** Construct `MemorySettings(neo4j=Neo4jConfig(uri=..., username=..., password=SecretStr(...)), embedding=EmbeddingConfig(provider=EmbeddingProvider.BEDROCK, model="amazon.titan-embed-text-v2:0", dimensions=1024, aws_region=REGION), extraction=ExtractionConfig(extractor_type=ExtractorType.NONE))`. The `embedding` argument must be an `EmbeddingConfig`, a provider string, or an `EmbeddingProvider` instance. A raw `BedrockEmbedder` is rejected because it does not satisfy the `EmbeddingProvider` protocol, so `EmbeddingConfig` with an explicit `aws_region` is the as-built path. No `llm=` is set: the core path is embedding-only and does manual writes, so the LLM extractor is not needed. The bolt path via the direct Python driver unlocks write-Cypher and `adopt_existing_graph`; the hosted NAMS path does not.
...
- [x] Extract `Lab_4_GraphRAG_Agent/lib/graphrag_agent.py`; point `agentcore_deploy/agent.py` and Lab 5 at it; leave notebook 01 inline. (`agent.py` imports `build_retrievers`/`make_tools`/`SYSTEM_PROMPT`; Lab 5 imports `build_graphrag_agent`.)
- [x] Apply the CONFIG.txt-else-`financial_data_load/.env` fallback loader to all 5 `data_utils.py` copies. (Implemented as `.env` → `CONFIG.txt` → `financial_data_load/.env`, consistent across all lab copies; `financial_data_load` prefers its own `.env`.)
- [x] Author the `Lab_5_Agent_Memory` notebooks on `neo4j-agent-memory`. Verified end-to-end 2026-07-11: both pass under `setup/run_notebooks.py --labs 5`. Fixed an embedder-config crash (`BedrockEmbedder` rejected by `MemorySettings`; replaced with `EmbeddingConfig(provider=BEDROCK, dimensions=1024, aws_region=REGION)`) and switched the deprecated `memory.graph.execute_read` to `memory.query.cypher`. Lab 5 registered in `setup/run_notebooks.py`.
- [ ] Fix the empty recall demo in `Lab_5_Agent_Memory/02_long_term_memory.ipynb`: `search_entities` and `long_term.get_context` return nothing at the default `threshold=0.7` when "Apple" is the only stored entity. Lower the threshold, adjust the query text, or reorder `adopt_existing_graph` before the recall.
