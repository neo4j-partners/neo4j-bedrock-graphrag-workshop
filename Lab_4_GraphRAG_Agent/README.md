...
> **Note:** This lab uses the same Neo4j Aura instance from the earlier labs. The knowledge graph — including document chunks, chunk embeddings, and the `chunkEmbeddings` vector index — must already be loaded. New to Strands agents? See the [Appendix - What Is an Agent?](../Appendix_What_Is_An_Agent) for the basics.
