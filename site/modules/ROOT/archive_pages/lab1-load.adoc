...
Creates relationships between nodes. The competitor and partner loads use MERGE on the target company name since they may reference companies mentioned in filings that are not in the primary dataset.
...
NOTE: The Company count is higher than 6 because the COMPETES_WITH and PARTNERS_WITH relationship loads use `MERGE (b:Company {name: row.targetCompanyName})`, which creates new Company nodes for competitors and partners mentioned in filings (e.g., Google, Samsung, OpenAI) that are not themselves in the seed data. These "mentioned companies" have only a `name` property — no `companyId`, `ticker`, or other identifiers.
