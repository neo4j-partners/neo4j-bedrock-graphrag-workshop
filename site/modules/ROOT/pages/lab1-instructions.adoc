...
Follow these steps to sign up for Neo4j Aura, create your database instance, and load the complete SEC financial knowledge graph, including the structured entities and the unstructured chunk/embedding half.
...
=== Option B: Free Signup (For self-paced learning)

If you are completing this lab independently or your organizer has instructed you to create a free account:
...
+
image::lab1-free-trial-region.png[AuraDB Free tier selection]
5. Your free instance is created automatically. **Save your credentials immediately.** Click **Download to continue**. The password is only shown once.
+
image::lab1-free-trial-credentials.png[Free instance credentials]

NOTE: If you signed up using the Free Signup option, your instance was already created during signup. Skip ahead to <<Part 3: Load the Knowledge Graph>>.
...
2. Select the **AuraDB Free** tier. This is a permanent free instance with no expiry. Give your instance a unique name based on your name (e.g., `ryans-lab-instance`).
+
image::lab1-configure-instance.png[Configure Instance]
3. Click **Create** to provision your database instance

NOTE: You do not need to choose a size, tier, or add-ons for the free instance. The Graph Analytics plugin used in xref:lab1-explore.adoc[the exploration step] is enabled later from your instance settings, when you need it.
...
So far you have loaded the **structured half**: companies, products, risk factors, and the relationships between them. Now you add the **unstructured half**: the SEC filing text split into chunks, each carrying a pre-computed 1024-dimensional embedding.
