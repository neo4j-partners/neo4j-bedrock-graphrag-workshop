...
In this lab, you will set up your Neo4j Aura database through the AWS Marketplace, restore the knowledge graph from a backup, and explore your graph visually.
...
![Backup & Restore Menu](images/backup_restore.png)

3. Click **Upload backup** to open the upload dialog, then drag the backup file into the dialog:

   ![Restore Dialog](images/restore_drag.png)

   **Use the pre-built backup file:**
   - Download `finance_data.backup` from the `data/` folder in this lab
   - Drag the file into the upload dialog
...
- Pre-configured vector index: `chunkEmbeddings`
